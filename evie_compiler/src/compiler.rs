...
impl Upvalue {
    pub fn new(index: ByteUnit, is_local: bool) -> Self {
...
#[allow(dead_code)]
impl<'a> Compiler<'a> {
    pub fn new(tokens: &'a [Token], allocater: &'a ObjectAllocator) -> Self {
    ...
    pub fn new_with_type(
        ...
        Compiler::new_with_type_and_writer(tokens, function_type, None, allocater)
    }
    ...
    pub fn new_with_type_and_writer(
        ...
        let mut c = Compiler {
            ...
            token_index: 0,
            parse_rules: Vec::new(),
            state: State::new(script_fn, Scope::new(), function_type),
    ...
    fn resolve_upvalue(&mut self, name: &Token) -> Result<Option<ByteUnit>> {
    ...
    fn resolve_local(&self, name: &Token) -> Result<Option<ByteUnit>> {
        Compiler::resolve_local_with_state(name, &self.state)
    }

    fn define_variable(&mut self, byte_unit: ByteUnit) {
        if self.current_scope_mut().depth == GLOBAL_SCOPE_DEPTH {
            self.emit_opcode_and_bytes(Opcode::DefineGlobal, byte_unit);
        } else {
            self.mark_initialized();
        }
    }
    ...
    fn argument_list(&mut self) -> Result<ByteUnit> {
        let mut count = 0;
        while self.current().token_type != TokenType::RightParen {
            self.expression()?;
            count += 1;
    ...
    #[inline]
    fn add_constant(&mut self, value: Value) -> ByteUnit {
    ...
    fn parse_variable(&mut self, message: &str) -> Result<ByteUnit> {
    ...
    #[inline]
    fn emit_byte(&mut self, byte: ByteUnit) {
        let mut line = 0;
    ...
    #[inline]
    fn emit_opcode_and_bytes(&mut self, op_code: Opcode, byte_unit: ByteUnit) {
        self.emit_op_code(op_code);
        self.emit_byte(byte_unit);
    }
...
#[inline]
fn as_two_bytes(jump: usize) -> (ByteUnit, ByteUnit) {
    let first = ((jump >> 8) & 0xff) as ByteUnit;
    let second = (jump & 0xff) as ByteUnit;
    (first, second)
}
...
mod tests {
    ...
    fn number() -> Result<()> {
        ...
        let mut buf = vec![];
        let compiler = Compiler::new_with_type_and_writer(
            tokens,
            FunctionType::Script,
            Some(&mut buf),
            &allocator,
...
"#,
    ...
    fn unary() -> Result<()> {
        ...
        let _function = compiler.compile()?;
        assert_eq!(
            r#"== <fn script> ==
0000 0001 OpCode[Constant]                  0 '3.14'
0002    | OpCode[Negate]
...
0002    | OpCode[Constant]                  1 ' '
0004    | OpCode[Add]
0005    | OpCode[GetLocal]                  1
0007    | OpCode[Add]
0008    | OpCode[Print]
0009 0005 OpCode[Nil]
0010    | OpCode[Return]
== <fn script> ==
