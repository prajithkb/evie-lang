//! Scanner module is responsible for generating the [Token]s
use evie_common::{error, errors::*};
use std::collections::HashMap;

use super::tokens::{Literal, Token, TokenType};

/// Scanner for Evie. Outputs the tokens a [Vec].
pub struct Scanner {
    source: String,
    source_len: usize,
    tokens: Vec<Token>,
    ...
    start: usize,
    current: usize,
    reserved_key_words: HashMap<&'static str, TokenType>,
}
...
impl Scanner {
    pub fn new(source: String) -> Self {
        ...
        Scanner {
            ...
            current: 0,
            // reserved keywords
            reserved_key_words: HashMap::from([
                ("and", TokenType::And),
    ...
    fn add_identifier(&mut self) {
        ...
        if let Some(v) = self.reserved_key_words.get(text.as_str()) {
            let t = *v;
...
mod tests {
    ...
    //TODO: add more tests for all kinds of tokens
